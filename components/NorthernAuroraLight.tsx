...
interface NorthernAuroraLightProps {
  variant?: 'northern' | 'brown';
}

export default function NorthernAuroraLight({ variant = 'brown' }: NorthernAuroraLightProps) {
    ...
    function draw() {
      ...
      for (let x = 0; x < cols; x++) {
        for (let y = 0; y < rows; y++) {
          ...
          if (variant === 'northern') {
            // Northern Lights - same formula for light mode (brightened)
            const redBase = Math.round((-20 * xmove) + (80 * ymove) + (50 - (0.6 * y * heightColorScaling)) + 150);
            const greenBase = Math.round((180 * xmove) + (20 * ymove) - 60 + (0.4 * y * heightColorScaling) + 200);
            const blueBase = Math.round((50 * xmove) + (30 * ymove) + (40 - (0.5 * y * heightColorScaling)) + (0.5 * y * heightColorScaling) + 150);
            
            red = Math.max(0, Math.min(255, redBase));
            green = Math.max(0, Math.min(255, greenBase));
            blue = Math.max(0, Math.min(255, blueBase));
          } else {
            // Honey Dawn palette (light mode) with olive haze anchor
            // Colors: #C97A40 (terracotta), #E3B887 (soft gold), #DAD3C0 (olive haze)
            // Creates warm honey/beige tones with subtle dusty sage
            red = Math.max(0, Math.min(255, Math.round((120 * xmove) + (140 * ymove) + (230 - 0.3 * y * heightColorScaling))));
            green = Math.max(0, Math.min(255, Math.round((100 * xmove) + (110 * ymove) + (210 - 0.25 * y * heightColorScaling))));
            blue = Math.max(0, Math.min(255, Math.round((80 * xmove) + (90 * ymove) + (185 - 0.2 * y * heightColorScaling))));
          }
          
          const opacity = variant === 'northern' ? 0.8 : 0.4;
    ...
    <div 
      ...
      style={{ 
        background: variant === 'northern' 
          ? 'linear-gradient(to bottom, #E0F2F5 0%, #FFFFFF 100%)'
          : 'linear-gradient(to bottom, #FFF6EB 0%, #EBD1B0 50%, #F5E4CB 100%)', 
        zIndex: -1 
      }}
    >
      <canvas 
